...
通用 ComfyUI API 节点包，当前包含文本 API 与柏拉图图片 API 两部分：
...
- 文本配置与图片配置完全分离
- 文本服务商列表不会被图片节点污染
- 图片平台使用独立配置文件保存模型列表和基础地址

### 柏拉图图片API管理

打开方式：

- 在 ComfyUI 画布右键菜单中选择 `Hhh 柏拉图图片API管理`
- 或添加 `Hhh 柏拉图图片API管理` 节点后点击按钮
...
适用于柏拉图平台中的 `gpt-image-*` 系列模型。

当前参数：

| 参数 | 说明 |
| --- | --- |
| `模型` | 选择 GPT 图片模型，例如 `gpt-image-2`。 |
| `提示词` | 文生图或图生图提示词。 |
| `质量` | `low` / `medium` / `high` / `auto`。 |
| `画幅比例` | 只需要选比例，节点会自动映射到稳定尺寸。 |
| `生成数量` | 请求生成张数；若平台一次只返回一张，节点会自动补齐请求。 |
| `随机种子` | `-1` 表示不传。 |
| `超时秒数` | 单次请求超时设置。 |
| `输出格式` | `png` / `jpeg` / `webp`。 |
| `保真度` | 图生图场景使用，支持 `low` / `high`。 |
| `参考图1-5` | 可选。接入后自动切换到图生图。 |
| `遮罩` | 可选。用于编辑模式。 |

说明：

- 不再暴露“像素尺寸”控件，避免与比例选择冲突。
- `画幅比例=9:16` 等场景会自动映射到对应尺寸，而不是固定退回 `1:1`。
- 节点输出是 `IMAGE batch`，多图返回时不会只保留第一张。

### Hhh 柏拉图 Nano

适用于柏拉图平台中的 `nano-banana` / `nano-banana-pro` / `nano-banana-2` 等模型。
...
| `模型` | 选择 Nano 图片模型。 |
| `提示词` | 文生图或图生图提示词。 |
| `画幅比例` | 柏拉图 Nano 系列比例参数。 |
| `图像规格` | `1K` / `2K` / `4K` / `auto`。 |
| `超时秒数` | 单次请求超时设置。 |
| `参考图1-5` | 可选。接入后自动切换到编辑模式。 |
| `遮罩` | 可选。用于编辑模式。 |

说明：

- Nano 节点和 GPT 节点共用同一份柏拉图图片平台密钥。
- 但它们的模型列表与参数模板是分开的。
...
- 柏拉图图片节点按平台做专用节点，不放进当前通用文本 API 节点。
- 图片节点与文本节点使用独立配置中心，但共享同一个 secrets 文件保存不同作用域的 API Key。
...
- `hhhapi_config.json`
- `hhhapi_image_config.json`
- `hhhapi_secrets.json`

`hhhapi_config.json` 存储供应商、模型、profile。  
`hhhapi_image_config.json` 存储柏拉图图片平台基础地址、超时和图片模型列表。  
